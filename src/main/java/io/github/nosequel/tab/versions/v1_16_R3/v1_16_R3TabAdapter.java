package io.github.nosequel.tab.versions.v1_16_R3;
...
public class v1_16_R3TabAdapter extends TabAdapter {
    ...
    public TabAdapter hideRealPlayers(Player player) {
        for(Player target : SharkHub.getInstance().getOnlinePlayers()) {
    ...
    public TabAdapter showRealPlayers(Player player) {
        if (!this.initialized.contains(player)) {
            ChannelPipeline pipeline = this.getPlayerConnection(player).networkManager.channel.pipeline();
        ...
        return new ChannelDuplexHandler() {
            ...
            public void write(ChannelHandlerContext context, Object packet, ChannelPromise promise) throws Exception {
                if (packet instanceof PacketPlayOutNamedEntitySpawn) {
                    ...
                    Field uuidField = entitySpawn.getClass().getDeclaredField("b");

                    uuidField.setAccessible(true);

                    Player target = Bukkit.getPlayer((UUID) uuidField.get(entitySpawn));
