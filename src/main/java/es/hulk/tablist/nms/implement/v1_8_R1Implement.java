 ...
 */
public class v1_8_R1Implement implements NMS {
    ...
    public TabEntry createEntry(Tablist tablist, String name, TabColumn column, int slot, int rawSlot, boolean legacy) {
        ...
        sendPacket(tablist.getPlayer(), new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.ADD_PLAYER, entity));
    ...
    public void updateText(Tablist tablist, TabEntry entry, String[] newStrings) {
        ...
        sendPacket(tablist.getPlayer(), new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.UPDATE_DISPLAY_NAME, entity));
    }
    ...
    public void updateTexture(Tablist tablist, TabEntry entry, Skin skin) {
        ...
        sendPacket(tablist.getPlayer(), new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.REMOVE_PLAYER, entity));
        sendPacket(tablist.getPlayer(), new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.ADD_PLAYER, entity));
    }
    ...
    public void updatePing(Tablist tablist, TabEntry entry, int ping) {
        ...
        sendPacket(tablist.getPlayer(), new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.UPDATE_LATENCY, entity));
    }

    @Override
    public void updateHeaderAndFooter(Player player, List<String> header, List<String> footer) {
        Packet packet = new PacketPlayOutPlayerListHeaderFooter(ChatSerializer.a("{\"text\":\"" + StringEscapeUtils.escapeJava(this.getListFromString(CC.translate(header))) + "\"}"));

        try {
            Field footerField = packet.getClass().getDeclaredField("b");

            footerField.setAccessible(true);
            footerField.set(packet, ChatSerializer.a("{\"text\":\"" + StringEscapeUtils.escapeJava(this.getListFromString(CC.translate(footer))) + "\"}"));
